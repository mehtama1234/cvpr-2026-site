"""Build the fifth CVPR interactive demo wave."""
import json
...
THIRD_WAVE = ROOT / "analysis/cvpr_interactive_third_wave/registry.json"
FOURTH_WAVE = ROOT / "analysis/cvpr_interactive_fourth_wave/registry.json"
ANALYSIS = ROOT / "analysis/cvpr_interactive_fifth_wave"
BASE = ROOT / "source-code/learning/cvpr-interactive-fifth-wave"
...
export function waveGate(summary) {
  ...
  if (summary.wave !== "fifth-interactive-wave") return "block";
...
console.log("ok cvpr-interactive-fifth-wave:", summary.demos, "demos");
"""
...
def pick_fifth_wave(backlog_rows, second_wave, third_wave, fourth_wave):
    converted = (
        {row["jobId"] for row in second_wave["waveRows"]}
        | {row["jobId"] for row in third_wave["waveRows"]}
        | {row["jobId"] for row in fourth_wave["waveRows"]}
    )
    seen = set()
    rows = []
...
def artifact_payload(kind, row):
    ...
    if kind == "smokeJson":
        return json.dumps({
            ...
            "viewer": f"fifth-wave-{row['theme']}-{row['repo']}",
...
def build_rows(backlog_rows, second_wave, third_wave, fourth_wave):
    rows = []
    for row in pick_fifth_wave(backlog_rows, second_wave, third_wave, fourth_wave):
        ...
        demo_id = f"fifth-wave-{row['theme']}-{row['repo'].lower().replace('_', '-').replace('.', '-')}"
        rows.append({
            "demoId": demo_id,
            "jobId": row["jobId"],
            "theme": row["theme"],
            ...
            "artifacts": artifacts,
            "outputFixture": f"Cached fifth-wave output for {row['repo']} from {row['theme']} theme.",
...
def build_package(rows, summary):
    ...
    write(BASE / "README.md", "# CVPR Interactive Fifth Wave\n\nFifth interactive wave converting the final promoted repo per theme into local-artifact-backed interactive demo state.\n")
