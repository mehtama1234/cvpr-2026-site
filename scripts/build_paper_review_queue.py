...
<body><header><div class="wrap"><div class="bug">CVPR 2026 · paper review</div><h1>Paper Review Queue</h1><p>A smaller review surface for checking whether the new paper notes are truly first-principles, detailed, and plain. Each card shows the hidden quantity, evidence, mathematical rule, naive failure, proof test, and counterexample.</p><nav><a href="index.html">all themes</a><a href="search.html">all papers</a><a href="paper-review.html">review any paper</a><a href="paper-note-quality.html">quality audit</a><a href="first-principles-audit.html">first-principles audit</a><a href="math.html">math pages</a></nav></div></header>
<main class="wrap"><section class="stats"><div class="stat"><b>{len(chosen)}</b><span>review papers</span></div><div class="stat"><b>{len(records)}</b><span>source papers</span></div><div class="stat"><b>8</b><span>themes balanced</span></div><div class="stat"><b>7</b><span>depth fields per paper</span></div>{stat_cards}</section><section class="rubric"><h2>How to review a paper note</h2><p>Read the card as a claim about what the paper is really trying to recover, measure, control, or prove. A strong note should let a careful reader understand the paper without already knowing the subfield, and it should make clear what evidence would change the reader's mind.</p><ul><li><b>Hidden thing.</b> Name the thing the paper cannot observe directly: depth, identity, cause, motion, reward, uncertainty, intent, or the true clean signal behind corrupted measurements.</li><li><b>Evidence path.</b> Check that the note says which observations constrain that hidden thing: pixels, frames, rays, labels, masks, rewards, text, camera motion, timing, or held-out tests.</li><li><b>Mathematical rule.</b> Look for a concrete relation, not a slogan: minimize this error, preserve this distance, match these distributions, update this belief, conserve this identity through time.</li><li><b>Naive failure.</b> Ask what simple version would do wrong. If the note cannot name the failure, it probably has not reached the real problem yet.</li><li><b>Proof test.</b> Ask what result would genuinely support the claim: a new view, a shifted domain, an ablation, a stress case, a calibrated uncertainty score, or a counterfactual edit.</li><li><b>Counterexample.</b> Mark rewrite if the note would still sound correct when the method is actually using a shortcut, leaking labels, memorizing templates, or passing only the easiest split.</li></ul></section><section class="state-tools"><h2>Save or move your review state</h2><p>Statuses live in this browser. Export them before switching machines, or paste a saved state here to restore a review session.</p><div class="state-grid"><label>Review state JSON or rewrite queue<textarea id="stateBox" placeholder="{{&quot;paper-1&quot;:&quot;rewrite&quot;,&quot;paper-2&quot;:&quot;ok&quot;}}"></textarea></label><div class="state-buttons"><button type="button" id="nextTodo">Next todo</button><button type="button" id="nextNeedsWork">Next needs work</button><button type="button" id="exportState">Export state</button><button type="button" id="exportRewriteQueue">Export rewrite queue</button><button type="button" id="importState">Import state</button><button type="button" id="clearState">Clear state</button></div></div><div class="state-note" id="stateNote">No state exported yet.</div></section><section class="controls"><label>Search<input id="q" placeholder="title, evidence, failure, tag"></label><label>Theme<select id="theme"><option value="">All themes</option>{options}</select></label><label>Status<select id="statusFilter"><option value="">All statuses</option><option value="todo">todo</option><option value="ok">clear</option><option value="rewrite">needs rewrite</option><option value="uncertain">uncertain</option></select></label><div class="count" id="count"></div></section><section class="queue" id="queue">{cards}</section></main>
