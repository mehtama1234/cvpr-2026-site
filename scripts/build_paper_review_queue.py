...
const params=new URLSearchParams(location.hash.replace(/^#/,"")); q.value=params.get("q")||""; theme.value=params.get("theme")||"";
function syncUrl(){{ const p=new URLSearchParams(); if(q.value.trim())p.set("q",q.value.trim()); if(theme.value)p.set("theme",theme.value); const next=p.toString()?`${{location.pathname}}#${{p}}`:location.pathname; history.replaceState(null,"",next); }}
...
q.addEventListener("input", ()=>{{ syncUrl(); applyFilters(); }}); theme.addEventListener("change", ()=>{{ syncUrl(); applyFilters(); }}); applyFilters();
</script></body></html>"""
