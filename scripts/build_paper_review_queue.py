...
function exportState(){{ stateBox.value=JSON.stringify(saved, null, 2); stateNote.textContent=`exported ${{Object.keys(saved).length}} saved statuses`; }}
function importState(){{ try{{ const incoming=JSON.parse(stateBox.value||"{{}}"), allowed=new Set(["todo","ok","rewrite","uncertain"]); for (const [key,value] of Object.entries(incoming)){{ if(!/^paper-\\d+$/.test(key)||!allowed.has(value)) throw new Error("invalid review state"); }} for (const key of Object.keys(saved)) delete saved[key]; Object.assign(saved, incoming); saveState(); for (const sel of statusSelects) setCardStatus(sel, saved[sel.dataset.paper]||"todo"); applyFilters(); stateNote.textContent=`imported ${{Object.keys(saved).length}} saved statuses`; }}catch(err){{ stateNote.textContent="import failed: paste the exported JSON object"; }} }}
function clearState(){{ for (const key of Object.keys(saved)) delete saved[key]; saveState(); for (const sel of statusSelects) setCardStatus(sel, "todo"); stateBox.value=""; applyFilters(); stateNote.textContent="cleared saved statuses"; }}
...
document.getElementById("exportState").addEventListener("click", exportState); document.getElementById("importState").addEventListener("click", importState); document.getElementById("clearState").addEventListener("click", clearState);
</script></body></html>"""
