...
<style>:root{{--ink:#101719;--paper:#F5F6F4;--panel:#FBFCFB;--line:#D7DCD9;--muted:#59656A;--accent:#0E7C86;--accent-deep:#0A5A62;--warn:#B37A1E;--mono:ui-monospace,"SF Mono",Menlo,Consolas,monospace;--sans:-apple-system,BlinkMacSystemFont,"Segoe UI",system-ui,Arial,sans-serif}}*{{box-sizing:border-box}}body{{margin:0;background:var(--paper);color:var(--ink);font-family:var(--sans);line-height:1.58}}.wrap{{max-width:1180px;margin:0 auto;padding:0 24px}}header{{background:var(--ink);color:#E7ECED;padding:42px 0 34px}}.bug,.meta,.stat span,nav a,summary,label,.count,.links,.rubric li{{font-family:var(--mono)}}.bug{{font-size:11px;letter-spacing:.18em;text-transform:uppercase;color:#4FC4CE}}h1{{font-size:42px;line-height:1.05;margin:10px 0}}header p{{max-width:88ch;color:#AEBABD}}nav a{{font-size:12px;color:#B7DDE1;margin-right:12px}}a{{color:var(--accent-deep)}}.stats{{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:18px 0}}.stat,.rubric{{background:var(--panel);border:1px solid var(--line);border-radius:8px}}.stat{{padding:11px}}.stat b{{display:block;font-size:24px}}.stat span{{font-size:11px;color:var(--muted)}}.rubric{{border-left:4px solid var(--warn);border-radius:0 8px 8px 0;padding:14px 16px;margin:16px 0}}.rubric h2{{font-size:20px;margin:0 0 8px}}.rubric p{{font-size:14.5px;color:#23302C;margin:7px 0}}.rubric ul{{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:10px 0 0;padding:0;list-style:none}}.rubric li{{font-size:12px;color:#23302C;background:#fffdf4;border:1px solid var(--line);border-radius:6px;padding:8px}}.controls{{position:sticky;top:0;z-index:4;background:rgba(245,246,244,.97);border-bottom:1px solid var(--line);padding:12px 0;display:grid;grid-template-columns:1fr 220px 180px auto;gap:10px;align-items:end}}label{{display:block;font-size:11px;color:var(--muted);letter-spacing:.06em;text-transform:uppercase}}input,select{{width:100%;border:1px solid var(--line);border-radius:6px;background:#fff;color:var(--ink);font:14px var(--sans);padding:8px 9px;margin-top:4px}}.count{{font-size:12px;color:var(--accent-deep);white-space:nowrap;padding-bottom:9px}}.queue{{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;margin:18px 0 36px}}.card{{background:var(--panel);border:1px solid var(--line);border-left:4px solid var(--accent);border-radius:8px;padding:15px 16px}}.card[data-status="ok"]{{border-left-color:#2F7A4F}}.card[data-status="rewrite"]{{border-left-color:#B37A1E}}.card[data-status="uncertain"]{{border-left-color:#6B5BA7}}.card[hidden]{{display:none}}.meta{{display:flex;gap:9px;align-items:center;flex-wrap:wrap;font-size:11px;color:var(--muted)}}.meta span:first-child{{color:var(--accent-deep);font-weight:700}}.card h2{{font-size:19px;line-height:1.22;margin:7px 0 8px}}.one{{font-size:14.5px;color:#23302C}}.links{{font-size:12px;margin:8px 0}}.status-control{{max-width:220px;margin:8px 0}}.depth{{background:#fffdf4;border:1px solid var(--line);border-left:3px solid var(--warn);border-radius:0 8px 8px 0;padding:9px 11px;margin:10px 0}}.depth p{{font-size:13.5px;line-height:1.55;margin:6px 0}}details{{margin-top:10px}}summary{{cursor:pointer;color:var(--accent-deep);font-size:12px}}details p{{font-size:13.5px;color:#23302C}}.tag{{display:inline-block;font-family:var(--mono);font-size:10.5px;color:var(--muted);background:#E4ECEB;border-radius:4px;padding:1px 6px;margin:2px 3px 0 0}}footer{{border-top:1px solid var(--line);padding:22px 0 54px;color:var(--muted);font-family:var(--mono);font-size:12px}}@media(max-width:900px){{.queue,.stats,.controls,.rubric ul{{grid-template-columns:1fr}}h1{{font-size:34px}}.count{{padding-bottom:0}}}}</style></head>
...
const cards=[...document.querySelectorAll(".card")], q=document.getElementById("q"), theme=document.getElementById("theme"), statusFilter=document.getElementById("statusFilter"), count=document.getElementById("count");
const statusSelects=[...document.querySelectorAll(".status")], storageKey="cvpr-paper-review-status";
const saved=JSON.parse(localStorage.getItem(storageKey)||"{{}}");
function setCardStatus(sel, value){{ sel.value=value; sel.closest(".card").dataset.status=value; }}
for (const sel of statusSelects){{ setCardStatus(sel, saved[sel.dataset.paper]||"todo"); sel.addEventListener("change", ()=>{{ saved[sel.dataset.paper]=sel.value; localStorage.setItem(storageKey, JSON.stringify(saved)); setCardStatus(sel, sel.value); applyFilters(); }}); }}
