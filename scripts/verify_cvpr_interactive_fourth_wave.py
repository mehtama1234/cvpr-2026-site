"""Verify the fourth CVPR interactive demo wave."""
import json
from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent
REGISTRY = ROOT / "analysis/cvpr_interactive_fourth_wave/registry.json"
PACKAGE = ROOT / "source-code/learning/cvpr-interactive-fourth-wave"
...
def main():
    ...
    rows = data["waveRows"]
    assert summary["wave"] == "fourth-interactive-wave"
    ...
    page = (ROOT / "cvpr-interactive-fourth-wave.html").read_text(encoding="utf-8")
    for token in (
        "Interactive Fourth Wave",
    ...
    print(f"verified CVPR interactive fourth wave: {summary['demos']} demos, {summary['status']}")
