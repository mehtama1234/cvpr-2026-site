"""Verify the fifth CVPR interactive demo wave."""
import json
from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent
REGISTRY = ROOT / "analysis/cvpr_interactive_fifth_wave/registry.json"
PACKAGE = ROOT / "source-code/learning/cvpr-interactive-fifth-wave"
...
def main():
    ...
    rows = data["waveRows"]
    assert summary["wave"] == "fifth-interactive-wave"
    ...
    page = (ROOT / "cvpr-interactive-fifth-wave.html").read_text(encoding="utf-8")
    for token in (
        "Interactive Fifth Wave",
    ...
    print(f"verified CVPR interactive fifth wave: {summary['demos']} demos, {summary['status']}")
