"""Build the fourth CVPR interactive demo wave."""
import json
...
ANALYSIS = ROOT / "analysis/cvpr_interactive_fourth_wave"
BASE = ROOT / "source-code/learning/cvpr-interactive-fourth-wave"
...
export function waveGate(summary) {
  ...
  if (summary.wave !== "fourth-interactive-wave") return "block";
...
console.log("ok cvpr-interactive-fourth-wave:", summary.demos, "demos");
"""
...
def pick_fourth_wave(backlog_rows, second_wave, third_wave):
    converted = {row["jobId"] for row in second_wave["waveRows"]} | {row["jobId"] for row in third_wave["waveRows"]}
    seen = set()
    rows = []
...
def artifact_payload(kind, row):
    ...
    if kind == "smokeJson":
        return json.dumps({
            ...
            "viewer": f"fourth-wave-{row['theme']}-{row['repo']}",
...
def build_rows(backlog_rows, second_wave, third_wave):
    rows = []
    for row in pick_fourth_wave(backlog_rows, second_wave, third_wave):
        ...
        demo_id = f"fourth-wave-{row['theme']}-{row['repo'].lower().replace('_', '-').replace('.', '-')}"
        rows.append({
            "demoId": demo_id,
            "jobId": row["jobId"],
            "theme": row["theme"],
            ...
            "artifacts": artifacts,
            "outputFixture": f"Cached fourth-wave output for {row['repo']} from {row['theme']} theme.",
...
def summarize(rows):
    summary = {
        ...
        "validator": "scripts/verify_cvpr_interactive_fourth_wave.py",
...
def main():
    ...
    rows = build_rows(backlog["backlogRows"], second_wave, third_wave)
    summary = summarize(rows)
    build_package(rows, summary)
    build_registry(rows, summary)
    build_page(rows, summary)
    print(f"wrote cvpr-interactive-fourth-wave.html: {summary['demos']} demos, status {summary['status']}")
