"""Build the second CVPR interactive demo wave."""
import json
from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent
BACKLOG = ROOT / "analysis/cvpr_interactive_expansion_backlog/registry.json"
ANALYSIS = ROOT / "analysis/cvpr_interactive_second_wave"
BASE = ROOT / "source-code/learning/cvpr-interactive-second-wave"
...
def pick_second_wave(backlog_rows):
    seen = set()
    rows = []
...
def artifact_payload(kind, row):
    ...
    if kind == "smokeJson":
        return json.dumps({
            ...
            "viewer": f"second-wave-{row['theme']}-{row['repo']}",
...
def build_rows(backlog_rows):
    rows = []
    for row in pick_second_wave(backlog_rows):
        ...
        demo_id = f"second-wave-{row['theme']}-{row['repo'].lower().replace('_', '-').replace('.', '-')}"
        rows.append({
            "demoId": demo_id,
            "jobId": row["jobId"],
            "theme": row["theme"],
            ...
            "artifacts": artifacts,
            "outputFixture": f"Cached second-wave output for {row['repo']} from {row['theme']} theme.",
...
def build_package(rows, summary):
    ...
    write(BASE / "README.md", "# CVPR Interactive Second Wave\n\nSecond interactive wave converting one additional promoted repo per theme into local-artifact-backed interactive demo state.\n")
